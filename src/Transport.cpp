...
/*static*/ uint32_t Transport::_requests_sent = 0;
/*static*/ uint32_t Transport::_responses_rx = 0;
/*static*/ uint32_t Transport::_responses_matched = 0;
					...
					if (packet.hops() != link_entry._remaining_hops) {
						...
						try {
							if (packet.data().size() == (Type::Identity::SIGLENGTH/8 + Type::Link::ECPUBSIZE/2) || packet.data().size() == (Type::Identity::SIGLENGTH/8 + Type::Link::ECPUBSIZE/2 + Type::Link::LINK_MTU_SIZE)) {
								...
								// recall() returns NONE for a transit link whose destination identity
								// we have not cached; get_public_key()/validate() on a NONE identity
								// dereferences a null object (a hard fault the surrounding try/catch
								// cannot absorb), so guard before touching it.
								if (peer_identity) {
