	...
	if (_object->_status != Type::Link::CLOSED && !(_object->_initiator && packet.context() == Type::Packet::KEEPALIVE && packet.data() == "\xFF")) {
		if (packet.receiving_interface() != _object->_attached_interface) {
			...
			if (packet.packet_type() == Type::Packet::DATA) {
				...
				{
					// (#60) Lossy-link recovery: if the LRRTT (ctx=254)
					// packet that promotes us to ACTIVE was dropped in
					// the air, but the sender's ADV reached us, the
					// link is *de-facto* established — the sender
					// wouldn't ADV without seeing our PROOF and
					// transitioning itself to ACTIVE. Promote our side
					// now and fire the link-established callback so the
					// owner can set resource_strategy / packet callback
					// before we evaluate the ADV.
					if (_object->_status == Type::Link::HANDSHAKE) {
						_object->_status = Type::Link::ACTIVE;
						_object->_activated_at = OS::time();
						_object->_last_proof = _object->_activated_at;
...
*/
				...
				// the part's map_hash with its own salt and silently
				// ignore parts that don't match its hashmap.
				case Type::Packet::RESOURCE:
				{
					for (auto& resource : _object->_incoming_resources) {
						const_cast<Resource&>(resource).on_part(packet);
					}
					break;
				}
				...
				case Type::Packet::RESOURCE_HMU:
				{
					...
					for (auto& resource : _object->_incoming_resources) {
						if (resource.hash() == resource_hash) {
							const_cast<Resource&>(resource).on_hashmap_update(plaintext);
							break;
						}
					}
					break;
